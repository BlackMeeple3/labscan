...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: ${C.bg}; color: ${C.text}; font-family: 'DM Sans', sans-serif; min-height: 100dvh; overscroll-behavior: none; -webkit-tap-highlight-color: transparent; }
  .app { max-width: 480px; margin: 0 auto; min-height: 100dvh; display: flex; flex-direction: column; }
  .header { padding: 14px 20px 10px; border-bottom: 1px solid ${C.border}; display: flex; align-items: center; gap: 10px; background: ${C.surface}; position: sticky; top: 0; z-index: 10; }
  .header-logo { font-family: 'JetBrains Mono', monospace; font-size: 13px; font-weight: 700; color: ${C.accent}; letter-spacing: 2px; }
  .screen { flex: 1; padding: 20px; display: flex; flex-direction: column; gap: 16px; padding-bottom: 90px; }
  .photo-zone { border: 2px dashed ${C.border}; border-radius: 16px; height: 190px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; cursor: pointer; background: ${C.surface}; position: relative; overflow: hidden; }
  ...
  .btn-primary { background: ${C.accent}; color: #fff; padding: 14px 20px; font-size: 15px; width: 100%; }
  .btn-secondary { background: ${C.card}; color: ${C.text}; padding: 12px 18px; font-size: 14px; border: 1px solid ${C.border}; width: 100%; }
  .btn-success { background: ${C.success}; color: #fff; padding: 14px 20px; font-size: 15px; width: 100%; }
  .btn-danger { background: ${C.danger}; color: #fff; padding: 12px 18px; font-size: 14px; }
  .btn-sm { background: ${C.card}; color: ${C.muted}; padding: 8px 14px; font-size: 12px; border: 1px solid ${C.border}; border-radius: 8px; cursor: pointer; font-family: 'DM Sans', sans-serif; font-weight: 600; display: inline-flex; align-items: center; gap: 6px; }
  ...
  .spinner { width: 44px; height: 44px; border: 3px solid ${C.border}; border-top-color: ${C.accent}; border-radius: 50%; animation: spin 0.8s linear infinite; }
  @keyframes spin { to { transform: rotate(360deg); } }
  .progress-bar { width: 200px; height: 4px; background: ${C.border}; border-radius: 2px; overflow: hidden; }
  .progress-fill { height: 100%; background: ${C.accent}; border-radius: 2px; transition: width 0.3s; }
  .sec-title { font-size: 11px; font-family: 'JetBrains Mono', monospace; color: ${C.muted}; letter-spacing: 1.5px; text-transform: uppercase; margin-bottom: 2px; }
  .sample-list { display: flex; flex-direction: column; gap: 8px; }
  .sample-card { background: ${C.card}; border: 1px solid ${C.border}; border-radius: 12px; padding: 13px 15px; cursor: pointer; transition: border-color 0.15s; -webkit-user-select: none; user-select: none; }
  .sample-card:active { background: #2a2f4a; }
  .sample-card.filled { border-color: ${C.success}; }
  .s-code { font-family: 'JetBrains Mono', monospace; font-size: 15px; font-weight: 700; color: ${C.accent}; display: flex; align-items: center; gap: 8px; }
  .s-dot { width: 8px; height: 8px; border-radius: 50%; background: ${C.border}; flex-shrink: 0; }
  .s-dot.on { background: ${C.success}; }
  .s-text { font-size: 12px; color: ${C.muted}; margin-top: 3px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .badge { font-size: 10px; padding: 2px 7px; border-radius: 20px; margin-left: auto; font-family: 'JetBrains Mono', monospace; }
  .badge-ok { background: #1a4a30; color: ${C.success}; }
  .badge-warn { background: #4a3a1a; color: ${C.warn}; }
  ...
  .sheet { background: ${C.surface}; border-radius: 20px 20px 0 0; padding: 20px; display: flex; flex-direction: column; gap: 14px; max-height: 94dvh; overflow-y: auto; animation: sUp 0.2s ease; }
  @keyframes sUp { from { transform: translateY(36px); opacity: 0; } }
  .handle { width: 40px; height: 4px; background: ${C.border}; border-radius: 2px; margin: 0 auto -6px; }
  .sh-title { font-family: 'JetBrains Mono', monospace; font-size: 16px; font-weight: 700; color: ${C.accent}; }
  .field-label { font-size: 11px; color: ${C.muted}; letter-spacing: 1px; text-transform: uppercase; font-family: 'JetBrains Mono', monospace; margin-bottom: 6px; }
  .divider { height: 1px; background: ${C.border}; }
  .row { display: flex; gap: 8px; }
  .f1 { flex: 1; }
  .chip-row { display: flex; flex-wrap: wrap; gap: 8px; }
  .chip { padding: 11px 14px; border-radius: 10px; border: 1px solid ${C.border}; background: ${C.card}; color: ${C.muted}; font-size: 13px; cursor: pointer; font-weight: 500; -webkit-user-select: none; user-select: none; flex: 1; text-align: center; min-width: 80px; transition: all 0.13s; }
  .chip.on { background: ${C.accentDim}; border-color: ${C.accent}; color: ${C.accent}; font-weight: 700; }
  .num-wrap { display: flex; align-items: center; background: ${C.card}; border: 1px solid ${C.border}; border-radius: 10px; overflow: hidden; }
  .num-btn { background: none; border: none; color: ${C.accent}; font-size: 22px; width: 52px; height: 52px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
  .num-btn:active { background: ${C.accentDim}; }
  .num-val { flex: 1; text-align: center; font-family: 'JetBrains Mono', monospace; font-size: 22px; font-weight: 700; color: ${C.text}; background: none; border: none; outline: none; }
  .num-unit { color: ${C.muted}; font-size: 13px; padding-right: 12px; flex-shrink: 0; }
  .prop-list { display: flex; flex-direction: column; gap: 6px; max-height: 240px; overflow-y: auto; }
  .prop-item { display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: 10px; background: ${C.card}; border: 1px solid ${C.border}; cursor: pointer; }
  .prop-item.on { border-color: ${C.accent}; background: #1e2540; }
  .prop-check { width: 20px; height: 20px; border-radius: 6px; border: 2px solid ${C.border}; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-size: 13px; }
  .prop-item.on .prop-check { background: ${C.accent}; border-color: ${C.accent}; color: #fff; }
  .toast { position: fixed; bottom: 24px; left: 50%; transform: translateX(-50%); background: ${C.success}; color: #fff; padding: 10px 20px; border-radius: 20px; font-size: 13px; font-weight: 600; z-index: 999; white-space: nowrap; animation: tIn 0.2s ease, tOut 0.2s ease 1.6s forwards; }
  @keyframes tIn { from { opacity: 0; transform: translateX(-50%) translateY(10px); } }
  @keyframes tOut { to { opacity: 0; } }
  .fab { position: fixed; bottom: 24px; right: 20px; width: 56px; height: 56px; border-radius: 50%; background: ${C.accent}; border: none; color: #fff; font-size: 24px; cursor: pointer; z-index: 50; box-shadow: 0 4px 20px rgba(79,142,247,0.4); display: flex; align-items: center; justify-content: center; }
  .fab:active { transform: scale(0.91); }
  textarea { background: ${C.card}; border: 1px solid ${C.border}; border-radius: 10px; color: ${C.text}; font-family: 'DM Sans', sans-serif; font-size: 14px; padding: 10px 12px; width: 100%; resize: none; outline: none; line-height: 1.5; }
  ...
  .mic-idle { background: ${C.card}; border: 2px solid ${C.border}; }
  .mic-rec { background: ${C.danger}; animation: pulse 0.85s ease infinite; }
  @keyframes pulse { 0%,100% { transform: scale(1); } 50% { transform: scale(1.1); } }
  .info-panel { background: ${C.bg}; border: 1px solid ${C.border}; border-radius: 12px; padding: 14px; display: flex; flex-direction: column; gap: 6px; }
  .info-panel-code { font-family: 'JetBrains Mono', monospace; font-size: 18px; font-weight: 700; color: ${C.accent}; }
  .info-panel-label { font-size: 10px; color: ${C.muted}; text-transform: uppercase; letter-spacing: 1px; font-family: 'JetBrains Mono', monospace; }
  .info-panel-val { font-size: 13px; color: ${C.text}; line-height: 1.5; white-space: pre-wrap; word-break: break-word; }
  .disc-item { display: flex; align-items: center; gap: 10px; padding: 12px 14px; border-radius: 12px; background: ${C.card}; border: 1px solid ${C.border}; cursor: pointer; transition: all 0.13s; }
  .disc-item.kept { border-color: ${C.success}; }
  .disc-item.gone { opacity: 0.33; border-style: dashed; border-color: ${C.danger}; }
  ...
  .disc-item.kept .disc-toggle { background: #1a4a30; color: ${C.success}; }
  .disc-item.gone .disc-toggle { background: #3a1a1a; color: ${C.danger}; }
  .sum-card { background: ${C.card}; border: 1px solid ${C.border}; border-radius: 14px; padding: 14px 16px; display: flex; flex-direction: column; gap: 5px; }
  .sum-code { font-family: 'JetBrains Mono', monospace; font-size: 14px; font-weight: 700; color: ${C.accent}; }
  .sum-rawtext { font-size: 11px; color: ${C.muted}; white-space: pre-wrap; word-break: break-word; margin-bottom: 4px; line-height: 1.4; }
  .sum-row { display: flex; justify-content: space-between; font-size: 12px; padding: 1px 0; }
  .sum-key { color: ${C.muted}; }
  .sum-val { font-family: 'JetBrains Mono', monospace; color: ${C.text}; font-weight: 600; font-size: 13px; }
  .sum-empty { color: ${C.border}; font-family: 'JetBrains Mono', monospace; }
  .sum-note { font-size: 12px; color: ${C.muted}; font-style: italic; margin-top: 2px; }
  .code-input-big { background: ${C.card}; border: 2px solid ${C.accent}; border-radius: 12px; padding: 14px 16px; font-family: 'JetBrains Mono', monospace; font-size: 24px; font-weight: 700; color: ${C.text}; text-align: center; letter-spacing: 3px; width: 100%; outline: none; }
  .code-preview { font-family: 'JetBrains Mono', monospace; font-size: 22px; font-weight: 700; color: ${C.accent}; text-align: center; letter-spacing: 3px; padding: 8px 0; }
  .info-tabs { display: flex; gap: 6px; }
  .info-tab { padding: 8px 14px; border-radius: 8px; border: 1px solid ${C.border}; background: ${C.card}; color: ${C.muted}; font-size: 12px; cursor: pointer; font-weight: 600; }
  .info-tab.on { background: ${C.accentDim}; border-color: ${C.accent}; color: ${C.accent}; }
  .sel-text-wrap { background: ${C.card}; border: 1px solid ${C.border}; border-radius: 10px; padding: 12px; font-size: 13px; color: ${C.text}; line-height: 1.8; white-space: pre-wrap; max-height: 180px; overflow-y: auto; }
  .word { cursor: pointer; border-radius: 4px; padding: 1px 3px; transition: background 0.1s; }
  .word.picked { background: ${C.accentDim}; color: ${C.accent}; }
`;
...
function extractSamplesFromLines(text) {
  return text.split("\n").map(l => l.trim()).filter(l => l.length > 1 && !isNoiseLine(l))
    .map((line, i) => {
      ...
      return { id: Date.now() + i, code: m ? m[1].toUpperCase() : null, rawText: line, data: null };
    });
}

function emptyData() {
  return { pesata: "", superficie: "", allestimento: null, volume: "", articoli: "", note: "" };
}
...
function useTesseract() {
  ...
  const recognize = useCallback(async (imageData, onProgress) => {
...
function NumPadInput({ value, onChange, unit = "", decimalDigits = 2 }) {
  ...
  function setInt(s) {
    if (!s && !decStr) { onChange(""); return; }
    onChange(s + (decStr ? "." + decStr : ""));
  }
  function setDec(s) {
    if (!s && !intStr) { onChange(""); return; }
    onChange((intStr || "0") + "." + s.slice(0, decimalDigits));
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      <div style={{ background: C.card, border: `2px solid ${C.accent}`, borderRadius: 12, padding: "10px 16px", textAlign: "center", fontFamily: "'JetBrains Mono', monospace", fontSize: 32, fontWeight: 700, letterSpacing: 2 }}>
        <span style={{ color: "#2ecc71" }}>{displayInt}</span>
        <span style={{ color: C.muted }}>,</span>
        <span style={{ color: "#7dcea0" }}>{displayDec}</span>
        {unit && <span style={{ fontSize: 15, color: C.muted, marginLeft: 8 }}>{unit}</span>}
      </div>
      <div style={{ display: "flex", gap: 10 }}>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
          <div style={{ fontSize: 10, color: C.muted, textAlign: "center", fontFamily: "'JetBrains Mono'", letterSpacing: 1, textTransform: "uppercase" }}>Interi</div>
          <MiniPad value={intStr} onChange={setInt} maxLen={6} color="#2ecc71" />
        </div>
        <div style={{ display: "flex", alignItems: "center", fontSize: 28, color: C.muted, fontFamily: "'JetBrains Mono'", fontWeight: 700, paddingTop: 24 }}>,</div>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
          <div style={{ fontSize: 10, color: C.muted, textAlign: "center", fontFamily: "'JetBrains Mono'", letterSpacing: 1, textTransform: "uppercase" }}>Decimali</div>
          <MiniPad value={decStr} onChange={setDec} maxLen={decimalDigits} color="#7dcea0" />
        </div>
      </div>
      {(intStr || decStr) && (
        <button className="btn-sm" style={{ alignSelf: "center" }} onClick={() => onChange("")}>✕ Azzera</button>
      )}
    </div>
  );
}
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 5 }}>
      {keys.map((k, i) => (
        <button key={i} onClick={() => k && press(k)} style={{
          height: 52, borderRadius: 10, border: k && k !== "⌫" ? `1px solid ${C.border}` : "none",
          background: k === "⌫" ? C.accentDim : k === "" ? "transparent" : C.card,
          color: k === "⌫" ? C.accent : C.text,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <div className="mic-row">
        <button className={`mic-btn ${listening ? "mic-rec" : "mic-idle"}`} onClick={toggleVoice}>
          {listening ? "⏹" : "🎤"}
        </button>
...
function InfoPanel({ sample }) {
  ...
  return (
    <div className="info-panel">
      <div className="info-panel-code">{sample.code || <span style={{ color: C.warn }}>Codice non trovato</span>}</div>
      {cols ? cols.filter(c => c.trim()).map((col, i) => (
        <div key={i}>
          <div className="info-panel-label">Campo {i + 1}</div>
          <div className="info-panel-val">{col.trim()}</div>
        </div>
      )) : (
        <div>
          <div className="info-panel-label">Informazioni riga</div>
          <div className="info-panel-val">{raw}</div>
        </div>
      )}
    </div>
  );
...
function DiscardScreen({ samples, onConfirm }) {
  ...
  return (
    <div className="screen">
      ...
      <div style={{ fontSize: 13, color: C.muted }}>Tocca per escludere. ✓ verde = tenuto, ✕ = scartato.</div>
        ...
        {samples.map(s => (
          <div key={s.id} className={`disc-item ${kept.has(s.id) ? "kept" : "gone"}`} onClick={() => toggle(s.id)}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="s-code" style={{ fontSize: 14 }}>{s.code || <span style={{ color: C.warn }}>⚠ Codice non trovato</span>}</div>
...
function SummaryScreen({ samples, imagePreview }) {
  const [copyDone, setCopyDone] = useState(false);
  const reportText = buildReportText(samples);

  function share() {
    if (navigator.share) {
      navigator.share({ title: "LAB·SCAN Rapporto", text: reportText }).catch(() => {});
    } else copyText();
  }
  ...
  return (
    <div className="screen">
      {imagePreview && (
        <div style={{ height: 70, borderRadius: 10, overflow: "hidden", flexShrink: 0 }}>
          <img src={imagePreview} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: 0.7 }} />
        </div>
      )}
      ...
      {samples.map(s => {
        ...
        return (
          <div key={s.id} className="sum-card">
            ...
            </> : <div style={{ fontSize: 12, color: C.muted, fontStyle: "italic" }}>Non compilato</div>}
          </div>
        );
      })}
    </div>
...
function ManualCodeOverlay({ onConfirm, onClose, recognize }) {
  ...
  async function handleInfoPhoto(e) {
    ...
    try {
      ...
      setExtractedText(text.trim()); setPickedWords([]);
  ...
  function toggleVoice() {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SR) { alert("Riconoscimento vocale non supportato."); return; }
    if (listening) { recogRef.current?.stop(); setListening(false); return; }
    const r = new SR(); r.lang = "it-IT"; r.continuous = true; r.interimResults = false;
    r.onresult = e => { const t = Array.from(e.results).map(x => x[0].transcript).join(" "); setVoiceNote(v => v ? v + " " + t : t); };
      ...
      <div className="sheet">
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: C.muted, marginTop: 6, textAlign: "center" }}>
            Digita le cifre → si completa automaticamente con zeri
          </div>
        </div>
        ...
        {infoTab === "photo" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {ocrLoading && <div style={{ color: C.muted, fontSize: 13, textAlign: "center" }}>Analisi OCR…</div>}
            {extractedText && !ocrLoading && <>
              <div style={{ fontSize: 11, color: C.muted }}>Tocca le parole da associare:</div>
              <div className="sel-text-wrap">
                {extractedText.split(/(\s+)/).map((token, i) => {
                  if (/^\s+$/.test(token)) return <span key={i}>{token}</span>;
                  return <span key={i} className={`word ${pickedWords.includes(token) ? "picked" : ""}`} onClick={() => toggleWord(token)}>{token}</span>;
                })}
              </div>
              {pickedWords.length > 0 && <div style={{ fontSize: 12, color: C.accent }}>✓ {pickedWords.join(" ")}</div>}
            </>}
          </div>
        )}
        {infoTab === "voice" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div className="mic-row">
              ...
              <span style={{ fontSize: 12, color: C.muted }}>{listening ? "In ascolto…" : "Oppure scrivi sotto"}</span>
            </div>
            <textarea rows={2} value={voiceNote} onChange={e => setVoiceNote(e.target.value)} placeholder="Nota sul campione…" />
          </div>
        )}
        {selectedInfo && <div style={{ fontSize: 12, background: C.card, borderRadius: 8, padding: "8px 12px", color: C.text, fontStyle: "italic" }}>📎 {selectedInfo}</div>}
        <div className="row">
          <button className="btn btn-secondary" style={{ width: 56 }} onClick={onClose}>✕</button>
          <button className="btn btn-primary f1" disabled={!isReady} onClick={() => {
            const code = padCode(trimmed);
            onConfirm({ code, rawText: selectedInfo ? `${code} — ${selectedInfo}` : code });
          }}>✓ Aggiungi {isReady ? preview : ""}</button>
        </div>
      </div>
    </div>
  );
...
function CompileOverlay({ sample, onSave, onClose, allSamples }) {
  ...
  if (phase === "confirm") return (
    <div className="overlay-bg">
      <div className="sheet">
        <div className="handle" />
        <div className="sh-title">Sovrascrivere?</div>
        <div style={{ background: C.card, border: `1px solid ${C.warn}`, borderRadius: 12, padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: C.warn }}>⚠️ Campione già compilato</div>
        ...
        <div style={{ color: C.muted, fontSize: 12, textAlign: "center" }}>{confirmQueue.length} rimanenti</div>
      </div>
    </div>
  );
  ...
  if (phase === "propagate") return (
    <div className="overlay-bg" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="sheet">
        <div className="handle" />
        <div className="sh-title">Applica ad altri?</div>
        {others.length === 0
          ? <div style={{ color: C.muted, fontSize: 13, textAlign: "center", padding: "16px 0" }}>Nessun altro campione</div>
          : <div className="prop-list">
              {others.map(s => (
                <div key={s.id} className={`prop-item ${selected.includes(s.id) ? "on" : ""}`}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 13, fontWeight: 700, color: C.accent }}>{s.code || "—"}</div>
                    <div style={{ fontSize: 11, color: C.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.rawText}</div>
                  </div>
      ...
      <div className="sheet">
          ...
          <div className="chip-row">
            {ALLESTIMENTI.map(a => (
              <div key={a} className={`chip ${d.allestimento === a ? "on" : ""}`}
                onClick={() => {
                  if (d.allestimento === a) { set("allestimento", null); set("volume", ""); }
                  else { set("allestimento", a); if (a === "Tasca") { set("superficie", "2"); set("volume", "100"); } }
                }}>
                {a}
              </div>
            ))}
          </div>
          {d.allestimento && <div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>Tocca di nuovo per deselezionare</div>}
        </div>

        <div>
          <div className="field-label">Pesata</div>
          <NumPadInput value={d.pesata} onChange={v => set("pesata", v)} unit="g" decimalDigits={2} />
        </div>
        ...
        {d.allestimento && (
          <div>
            ...
            {d.allestimento === "Tasca" && <div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>ℹ️ Default: 2 dm² / 100 ml</div>}
          </div>
        )}

        <div>
          <div className="field-label">Superficie</div>
          <NumInput value={d.superficie} onChange={v => set("superficie", v)} step={0.5} unit="dm²" />
        </div>

        <div>
          <div className="field-label">N° Articoli</div>
          <NumInput value={d.articoli} onChange={v => set("articoli", v)} step={1} unit="pz" />
        </div>

        <div>
          <div className="field-label">Note</div>
          <NotesInput value={d.note} onChange={v => set("note", v)} />
        </div>
...
export default function App() {
  ...
  function handleSave(updates) {
    setSamples(prev => { const m = new Map(updates.map(u => [u.id, u.data])); return prev.map(s => m.has(s.id) ? { ...s, data: m.get(s.id) } : s); });
    showToast("Salvato ✓");
  }
    ...
    <>
      ...
      <div className="app">
        ...
        {screen === "photo" && (
          <div className="screen">
            {!tessReady && (
              <div style={{ fontSize: 12, color: C.warn, textAlign: "center", padding: "8px 12px", background: C.card, borderRadius: 8 }}>
                ⏳ Caricamento motore OCR…
              </div>
            )}
            <div style={{ fontSize: 14, color: C.muted, lineHeight: 1.6 }}>Fotografa il documento. Ogni riga = un campione.</div>
            <div className="photo-zone" onClick={() => tessReady && fileRef.current?.click()}>
              ...
              <div className="photo-zone-overlay">
                ...
                <div style={{ fontSize: 13, color: C.muted }}>{tessReady ? "Tocca per fotografare" : "Caricamento OCR…"}</div>
              </div>
            </div>
            <input ref={fileRef} type="file" accept="image/*" capture="environment" style={{ display: "none" }} onChange={handlePhoto} />
            <button className="btn btn-primary" disabled={!tessReady} onClick={() => fileRef.current?.click()}>
              📷 Scatta / Carica foto
            </button>
          </div>
        )}
          ...
          <div className="screen">
            ...
            {samples.length > 0 ? <>
                ...
                {samples.map(s => (
                  <div key={s.id} className={`sample-card ${isDataFilled(s.data) ? "filled" : ""}`} onClick={() => setActiveSample(s)}>
                    <div className="s-code">
                      ...
                      {s.code || <span style={{ color: C.warn }}>⚠ Codice non trovato</span>}
              ...
              <div style={{ textAlign: "center", padding: "32px 0" }}>
                ...
                <div style={{ color: C.muted, fontSize: 14 }}>Nessun campione. Scatta una foto o aggiungi manualmente.</div>
              </div>
            )}
        ...
        {showManualCode && (
          <ManualCodeOverlay
            ...
            onConfirm={({ code, rawText }) => {
              setSamples(prev => [...prev, { id: Date.now(), code, rawText, data: null }]);
